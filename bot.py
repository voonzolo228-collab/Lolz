...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if args and args.startswith("deal_"):
        ...
        if deal["status"] != "pending":
            await message.answer(f"❌ Сделка уже {'занята' if deal['status'] == 'active' else 'завершена'}.", reply_markup=main_menu_kb())
            return
        ...
        await update_deal(deal["deal_id"], participant_id=message.from_user.id, participant_role=opposite, status="active")
        deal = await get_deal(deal["deal_id"])
        await log_action("DEAL", message.from_user.id, message.from_user.username, deal["deal_id"], "JOIN", f"Роль: {opposite}")
        try:
            await bot.send_message(deal["creator_id"], f"🎉 К вашей сделке <code>{deal['deal_id']}</code> присоединился участник!\n\n{deal_card(deal)}", reply_markup=main_menu_kb())
        except Exception:
            pass
        role_text = "Продавец" if opposite == "seller" else "Покупатель"
        await message.answer(f"✅ Вы успешно присоединились к сделке!\n\n📄 <b>Сделка {deal['deal_id']}</b>\n👤 Ваша роль: <b>{role_text}</b>\n💰 Сумма: <b>{deal['amount']} {deal['currency']}</b>\n📝 Описание: {deal['description']}\n\n⏳ Ожидайте подтверждения от второй стороны.", reply_markup=main_menu_kb())
        return
...
async def verification(call: types.CallbackQuery):
    ...
    await call.message.edit_text(f"✅ <b>Верификация</b>\n\nСтатус: <b>{status}</b>", reply_markup=back_main_kb())
    await call.answer()
