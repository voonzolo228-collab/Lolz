...
async def cmd_start(message: types.Message):
    try:
        ...
        # ЯКЩО Є ПАРАМЕТР deal_ - ПІДКЛЮЧЕННЯ ДО УГОДИ
        if args and args.startswith('deal_'):
            ...
            if not deal:
                await message.answer("❌ Сделка не найдена.", reply_markup=main_menu_inline())
                return
            if deal[3] is not None:
                await message.answer("❌ К этой сделке уже подключились.", reply_markup=main_menu_inline())
                return
            if user_id == deal[1]:
                await message.answer("❌ Вы создали эту сделку. Ожидайте подключения второй стороны.", reply_markup=main_menu_inline())
                return
