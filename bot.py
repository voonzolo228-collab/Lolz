...
def update_balance(user_id, currency, amount):
    """Оновлює баланс користувача. currency - назва валюти у нижньому регістрі"""
    if amount <= 0:
        return
    conn = sqlite3.connect('funpay.db')
    cur = conn.cursor()
...
async def confirm_transfer(msg: types.Message):
    ...
    if deal[4]:
        update_balance(deal[4], deal[6].lower(), deal[5])
    ...
    # Відправляємо профіт у групу (тільки якщо є опис і це посилання)
    if deal[7] and 't.me/nft/' in deal[7]:
        profit_data = await calculate_profit(deal[7])
        if not profit_data['error']:
            worker_name = 'admin'
            worker_share = f"{profit_data['profit']:.2f}"
            amount = f"{profit_data['price']:.2f}"
