...
# ==================== БАНЕРИ (ПОСИЛАННЯ) ====================
# Замініть ці посилання на свої прямі URL-адреси зображень або File ID
BANNER_MAIN = "https://i.imgur.com/YOUR_MAIN_BANNER.jpg"
BANNER_REQUISITES = "https://i.imgur.com/YOUR_REQ_BANNER.jpg"
BANNER_DEAL = "https://i.imgur.com/YOUR_DEAL_BANNER.jpg"
# ============================================================

EMOJI_STAR = '<tg-emoji emoji-id="5465438837335606404">⭐️</tg-emoji>'
EMOJI_SHIELD = '<tg-emoji emoji-id="5467617650890989047">🛡</tg-emoji>'
...
# --- ДОПОМІЖНА ФУНКЦІЯ ОНОВЛЕННЯ/НАДСИЛАННЯ ФОТО ---
async def send_or_update_photo(event: CallbackQuery | Message, photo_url: str, caption: str, reply_markup: InlineKeyboardMarkup = None):
    """Надсилає нове фото або видаляє попереднє текстове повідомлення та відправляє фото з підписом."""
...
async def choose_req_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_caption(caption=f"💳 Введите реквизиты для выбранного способа (<b>{req_type}</b>):", parse_mode=ParseMode.HTML)
...
async def admin_add_balance_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("👤 Введите <b>ID пользователя</b>:", parse_mode=ParseMode.HTML)
...
async def process_admin_amount_balance(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Пользователю <code>{user_id}</code> начислено <b>{amount} {EMOJI_STAR} STARS</b>.", parse_mode=ParseMode.HTML)
...
async def process_admin_amount_deals(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Пользователю <code>{user_id}</code> добавлено <b>{amount} сделок</b>.", parse_mode=ParseMode.HTML)

# --- ОСНОВНА ЛОГІКА СТВОРЕННЯ УГОДИ ---
...
async def show_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💳 <b>Реквизиты:</b> {req_status}\n"
        f"⭐ <b>Рейтинг:</b> 5.0 / 5.0\n\n"
...
async def start_create_deal(callback: CallbackQuery, state: FSMContext):
    ...
    text = "Выберите тип сделки:\n\n❗️ <b>Сделку создаёт продавец</b> ❗️"
...
async def process_warning(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        "Передайте в этой сделке, или перешлите сюда сам подарок.\n"
...
async def add_new_req_in_deal(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        "💳 <b>Настройка новых реквизитов</b>\n\n"
...
async def process_description(message: Message, state: FSMContext):
    ...
    summary_text = (
        ...
        f"<b>Метод оплаты:</b> 💳 {data['requisites']}\n"
...
async def process_payment(callback: CallbackQuery):
    ...
    pay_confirm_text = (
        f"✅ <b>Оплата сделки #{deal_id} успешно проведена!</b>\n\n"
