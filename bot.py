...
# --- ЗАПУСК ЧЕРЕЗ LONG POLLING (ОДРАЗУ ВІДПОВІДАЄ) ---
async def main():
    logging.basicConfig(level=logging.INFO)
    await db_module.init_db()
    
    # Видаляємо всі вебхуки, щоб бот працював через Polling без затримок
    await bot.delete_webhook(drop_pending_updates=True)
    
    print("Бот успішно запущений через Long Polling!")
