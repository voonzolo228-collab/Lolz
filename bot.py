...
async def get_user(user_id: int, username: str = None) -> Dict[str, Any]:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            await db.execute("INSERT INTO users (user_id, username, language, created_at) VALUES (?, ?, ?, ?)", 
                           (user_id, username or "", "ru", now))
            await db.commit()
        except Exception as e:
            logger.error(f"Не вдалося створити користувача {user_id}: {e}")
        return {"user_id": user_id, "username": username or "", "language": "ru", "verified": 0, 
                "balance_ton": 0, "balance_rub": 0, "balance_usd": 0, "balance_uah": 0, 
                "balance_stars": 0, "balance_usdt": 0, "balance_kgs": 0, "balance_uzs": 0, 
                "balance_byn": 0, "balance_kzt": 0, "requisites": "", "created_at": now}
...
async def create_deal(deal_id: str, creator_id: int, role: str, amount: float, currency: str, description: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    logger.info(f"✅ Создана угода: {deal_id}")
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if args and args.startswith("deal_"):
        ...
        # ============ СПОСІБ 3: Шукаємо без # (якщо раптом) ============
        if not deal:
            logger.info(f"🔍 Шукаємо без #: 716MZK{deal_code}")
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT * FROM deals WHERE deal_id LIKE ?", (f"%716MZK{deal_code}%",)) as cur:
                    row = await cur.fetchone()
                    if row:
                        deal = dict(row)
                        logger.info(f"✅ Знайдено угоду через без #: {deal['deal_id']}")
...
async def process_amount(message: types.Message, state: FSMContext):
    if message.text.startswith('/') or not message.text.strip():
        return
...
async def balance_info(call: types.CallbackQuery):
    ...
    if call.data == "topup_info":
        await call.message.edit_text(get_text(user_id, "topup_info"), parse_mode="HTML", reply_markup=back_main_kb())
    else:
        await call.message.edit_text(get_text(user_id, "withdraw_info"), parse_mode="HTML", reply_markup=back_main_kb())
    await call.answer()
...
async def support(call: types.CallbackQuery):
    user_id = call.from_user.id
    await call.message.edit_text(get_text(user_id, "support"), parse_mode="HTML", reply_markup=back_main_kb())
    await call.answer()
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                await session.get("https://loLz-4.onrender.com/health")
        except:
            pass
