...
def init_db():
    ...
    ''')
    cur.execute("CREATE INDEX IF NOT EXISTS idx_deals_creator ON deals(creator_id)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_deals_participant ON deals(participant_id)")
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_logs_deal ON logs(deal_id)")
    conn.commit()
    conn.close()
