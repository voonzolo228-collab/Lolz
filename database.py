    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT * FROM users WHERE user_id = ?", (user_id,)) as cursor:
            ...
            if not user:
                ...
                await db.commit()
                return {"user_id": user_id, "username": username, "deals_count": 0, "balance": 0, "referrals": 0, "lang": "Русский"}
            return {"user_id": user[0], "username": user[1], "deals_count": user[2], "balance": user[3], "referrals": user[4], "lang": user[5]}
