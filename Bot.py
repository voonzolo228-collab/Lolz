...
async def language_callback(callback_query: types.CallbackQuery):
    ...
    langs = ['🇬🇧 English', '🇷🇺 Русский', '🇺🇦 Українська', '🇨🇳 中文', '🇰🇿 Қазақша', '🇰🇷 한국어', '🇯🇵 日本語', '🇩🇪 Deutsch', '🇸🇦 العربية']
    for lang in langs:
        kb.add(InlineKeyboardButton(lang, callback_data=f"lang_{lang}"))
    kb.add(InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu"))
    await callback_query.message.edit_text("🌐 <b>Выберите язык</b>", parse_mode="HTML", reply_markup=kb)
    await callback_query.answer()
...
async def set_language_callback(callback_query: types.CallbackQuery):
    ...
    conn.commit()
    conn.close()
    await callback_query.message.edit_text(f"✅ Язык изменен на {lang_text}", reply_markup=back_menu_inline())
    await callback_query.answer()
...
async def cancel_deal(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("Создание сделки отменено.")
...
async def admin_back_to_menu(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("🏠 <b>Главное меню</b>", parse_mode="HTML", reply_markup=main_menu_inline())
